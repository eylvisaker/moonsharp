...
namespace MoonSharp.Interpreter
{
    ...
    public class Script : IScriptPrivateResource
    {
        ...
        /// <summary>
        /// Does the following: 
        ///   1. checks to see if expression is in the globals table. If it is:
        ///      1a. if it's a closure, call it and return the result.
        ///      1b. If it's a function, call it and return the result.
        ///      1c. returns it.
        ///   2. otherwise, calls DoString with ("return " + expression) as its parameter.
        /// </summary>
        /// <param name="expression"></param>
        /// <param name="parameters">Parameters to pass to the function. Ignored
        /// if evaluated as an expression.</param>
        /// <returns></returns>
        public DynValue CallOrEval(string expression, params object[] parameters)
        {
            object glob = this.Globals[expression];

            if (glob != null)
            {
                if (glob is Closure c)
                {
                    return c.Call(parameters);
                }

                DynValue global = glob as DynValue;

                if (global != null)
                {
                    if (global.Type == DataType.Function)
                        return Call(global);

                    return global;
                }

                if (glob is string s) return DynValue.NewString(s);
                if (glob is double d) return DynValue.NewNumber(d);
                if (glob is int i) return DynValue.NewNumber(i);
                if (glob is bool b) return DynValue.NewBoolean(b);

                return DynValue.FromObject(this, glob);
            }

            return DoString("return " + expression);
        }
